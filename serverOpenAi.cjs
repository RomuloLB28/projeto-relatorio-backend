...
const apiKey = ""; // CHAVE DA API DO CHATGPT
...
app.post("/send-email", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        // Envia o texto para a API da OpenAI
        const openAiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                model: "gpt-3.5-turbo",
                messages: [
                    { role: "system", content: "faça um relatorio do texto do pdf" },
        ...
        const chatGptResponse = chatResponse.choices[0].message.content;
...
rl.question("Digite sua mensagem: ", (chatGptResponse, tel) => {
    const data = {
        messaging_product: "whatsapp",
        to: tel,
        type: "text",
